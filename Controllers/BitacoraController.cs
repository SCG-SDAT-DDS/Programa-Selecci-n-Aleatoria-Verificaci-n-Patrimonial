...
namespace Transparencia.Controllers
{
    public class BitacoraController : Controller
    {
        ...
        public ActionResult Index(FiltrosPlantilla filtros, int PerPage = 10, int iPagina = 1)
        {
            ...
            var lstModel = db.Bitacora.Where(x=>
            ( filtros.UsuarioId == null || x.usuarioId == filtros.UsuarioId)
            && (filtros.PeriodoDesde == null || x.fechaCreacion >= filtros.PeriodoDesde)
            && (filtros.PeriodoHasta == null || x.fechaCreacion <= filtros.PeriodoHasta)).OrderByDescending(x=>x.fechaCreacion).ToPagedList(iPagina, PerPage);
